...
from kaneki import dp, TOKEN, bot
from kaneki.config import get_bool_key, get_list_key
from kaneki.modules import ALL_MODULES, LOADED_MODULES, MOD_HELP
...
if get_bool_key('LOAD_MODULES'):
    ...
    for module_name in modules:
        ...
        imported_module = import_module("kaneki.modules." + module_name)
...
import_module('kaneki.modules.pm_menu')
# Import misc stuff
import_module("kaneki.utils.exit_gracefully")
if not get_bool_key('DEBUG_MODE'):
    import_module("kaneki.utils.sentry")
...
import_module("kaneki.utils.db_structure_migrator")
...
if use_uvloop is True:
    log.info("Kaneki: Using uvloop to speedup.")
    uvloop.install()
