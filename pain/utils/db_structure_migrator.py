...
from pain import bot, OWNER_ID
from pain.services.mongo import mongodb
from pain.utils.logger import log
from pain.versions import DB_STRUCTURE_VER


async def notify_bot_owner(old_ver, new_ver):
    await bot.send_message(
        OWNER_ID,
        f"PAIN database structure was updated from <code>{old_ver}</code> to <code>{new_ver}</code>"
    )
...
if not (data := mongodb.db_structure.find_one({'db_ver': {"$exists": True}})):
    ...
    if DB_STRUCTURE_VER > curr_ver:
        ...
        while curr_ver < DB_STRUCTURE_VER:
            ...
            log.debug("Importing: pain.db." + str(new_ver))
            import_module("pain.db." + str(new_ver))
